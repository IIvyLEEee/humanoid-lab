...
# from ..tasks.locomotion.mdp import CustomUniformVelocityCommand
from ..tasks.mimic.mdp import MotionCommand
...
def attach_onnx_metadata(env: ManagerBasedRLEnv, run_path: str, path: str, filename="policy.onnx") -> None:
    ...
    anchor_body_id = env.scene["robot"].body_names.index(env.command_manager.get_term("motion").cfg.anchor_body_name)
    print("anchor_body_id:", anchor_body_id)
    print('env.observation_manager.active_terms["policy"]:\n', env.observation_manager.active_terms["policy"])
    # exit(-1)
    metadata = {
        "run_path": run_path,
        ...
        "anchor_body_id": env.scene["robot"].body_names.index(
            env.command_manager.get_term("motion").cfg.anchor_body_name
        ),
    }
...
class _OnnxLocomotionPolicyExporter(_OnnxPolicyExporter):
    def __init__(self, env: ManagerBasedRLEnv, actor_critic, normalizer=None, verbose=False):
        super().__init__(actor_critic, normalizer, verbose)
        cmd: CustomUniformVelocityCommand = env.command_manager.get_term("loco_command")
...
def attach_loco_onnx_metadata(env: ManagerBasedRLEnv, run_path: str, path: str, filename="policy.onnx") -> None:
    onnx_path = os.path.join(path, filename)

    # anchor_body_id = env.scene["robot"].body_names.index(env.command_manager.get_term("loco_command").cfg.anchor_body_name)
    # print('anchor_body_id:',anchor_body_id)
    print('env.observation_manager.active_terms["policy"]:\n', env.observation_manager.active_terms["policy"])
    # exit(-1)
    metadata = {
